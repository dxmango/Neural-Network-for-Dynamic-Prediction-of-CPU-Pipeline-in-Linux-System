        ...
        if(correct==1)
           {
               ...
               while(i<=hislen-1)
                { 
                //fprintf("hello weight");
                if(pred->neuralstruct.history[i]==1)
                pred->neuralstruct.weight[(baddr >> MD_BR_SHIFT) & (pred->btb.sets - 1)][i]=pred->neuralstruct.weight[(baddr >> MD_BR_SHIFT) & (pred->btb.sets - 1)][i]+3;
                if(pred->neuralstruct.history[i]==-1)
